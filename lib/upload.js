...
module.exports = async (processingConfig, tmpDir, axios, log, patchConfig) => {
  ...
  for (const file of files) {
    if (file.includes('.zip')) {
      if (processingConfig.downloadZip) {
  ...
  if (processingConfig.datasetMode === 'create') {
    if (processingConfig.dataset.id) {
      ...
      // permet de créer le jeu de donnée avec l'identifiant spécifié
    ...
    await patchConfig({ datasetMode: 'update', dataset: { id: metaDataset.id, title: metaDataset.title } })
    processingConfig.dataset = { id: metaDataset.id, title: metaDataset.title }
  ...
  for (const file of attachments) {
    ...
    try {
      ...
      const response = await axios({
        method: 'post',
        url: `api/v1/datasets/${metaDataset.id}/metadata-attachments`,
        data: formData,
      ...
      const attachment = await axios({
        method: 'get',
        url: `api/v1/datasets/${metaDataset.id}/`
      })
      const datasetAttachments = attachment.data.attachments || []
      const idx = datasetAttachments.findIndex(a => a.name === response.data.name)
      if (idx >= 0) datasetAttachments.splice(idx, 1)
      datasetAttachments.push({
        type: 'file',
  ...
  for (const dataset of datasets) {
    ...
    if (!processingConfig.dataset || !processingConfig.dataset.id) {
