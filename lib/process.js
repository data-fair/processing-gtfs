...
module.exports = async (tmpDir, log) => {
  ...
  const stopRoutes = []
  await stopTimes.forEach(stopTime => {
    const index = trips.findIndex(trip => trip.trip_id === stopTime.trip_id)
    if (index !== -1) {
      const index2 = stopRoutes.findIndex(stopRoute => stopRoute.stop_id === stopTime.stop_id)
      if (index2 === -1) {
        stopRoutes.push({
          stop_id: stopTime.stop_id,
          routes: [trips[index].route_id]
        })
      } else {
        if (!stopRoutes[index2].routes.includes(trips[index].route_id)) {
          stopRoutes[index2].routes.push(trips[index].route_id)
        }
      }
    }
  })
      ...
      transform: (line, _, next) => {
        ...
        if (line[0] === 'stop_id') {
          ...
          const stop = {
            ...
            routes: stopRoutes.find(stopRoute => stopRoute.stop_id === line[0]).routes.join(';')
          }
          stops.push(stop)
          next()
        }
