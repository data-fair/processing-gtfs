...
module.exports = async (tmpDir, log) => {
  ...
  let readStream = await fs.createReadStream(path.join(tmpDir, 'stop_times.txt'), { encoding: 'utf8' })
  const writeStream = await fs.createWriteStream(path.join(tmpDir, 'stop_times.csv'), { encoding: 'utf8' })
  let tripIndex, arrivalIndex, departureIndex, stopTripsIndex, stopSequenceIndex, pickupTypeIndex, dropOffTypeIndex
    ...
    new stream.Transform({
      ...
      transform: (line, _, next) => {
        ...
        if (line[0] === 'trip_id') {
          ...
          const indexHeadsign = currentTripHeadsign.findIndex(tripHeadsign => tripHeadsign.trip_id === line[tripIndex])
          if (indexHeadsign === -1) {
  ...
  readStream.close()
  readStream = await fs.createReadStream(path.join(tmpDir, 'stop_times.txt'), { encoding: 'utf8' })
  await pump(
    byline.createStream(readStream),
    new stream.Transform({
      objectMode: true,
      transform: (line, _, next) => {
        ...
        if (line[0] !== 'trip_id') {
          ...
          origin = tripHeadsigns.find(tripHeadsign => tripHeadsign.trip_id === line[tripIndex])
          destination = tripHeadsigns.find(tripHeadsign => tripHeadsign.trip_id === line[tripIndex])
        ...
        const item = {
          ...
          route_id: route ? route.route_short_name || route.route_long_name : 'route_id',
  ...
  stopTimes.forEach(stopTime => {
    ...
    stopTime.trip_ids.forEach(trip_id => {
      ...
      if (!stopRoutes[stopRoutes.length - 1].route_id.includes(route.route_short_name || route.route_long_name)) {
        stopRoutes[stopRoutes.length - 1].route_id.push(route.route_short_name || route.route_long_name)
      }
    })
  })
